...
/**
 * Maven to OSGi simple converter
 */
public class Maven2OsgiConverter {
     ...
     * exceptions
     * <ul>
     * <li>if artifact.getFile is not null and the jar contains a OSGi Manifest
     * with Bundle-SymbolicName property then that value is returned</li>
     ...
     * </ul>
     * @throws IOException cannot read the manifest
     */
    public String getBundleSymbolicName(Artifact artifact) throws IOException {
        if ((artifact.getFile() != null) && artifact.getFile().isFile()) {
            ...
            try {
                ...
                if (jar.getManifest() != null) {
                    String symbolicNameAttribute = jar.getManifest()
                            .getMainAttributes().getValue(
                                    "Bundle-SymbolicName");
                    Map bundleSymbolicNameHeader = analyzer
                            .parseHeader(symbolicNameAttribute);
        ...
        if ((i < 0) && (artifact.getFile() != null)
                && artifact.getFile().isFile()) {
            String groupIdFromPackage = getGroupIdFromPackage(artifact
                    .getFile());
        ...
        return getBundleSymbolicName(artifact.getGroupId(), artifact
                .getArtifactId());
    }
    ...
    private String getGroupIdFromPackage(File artifactFile) {
        try {
            ...
            for (Iterator it = packageNames.iterator(); it.hasNext();) {
                ...
                String[] packageNameSections = packageName.split("\\"
                        + FILE_SEPARATOR);
                ...
                {
                    /*
                     * find the common portion of current package and previous
                     * selected groupId
                     */
                    int i;
                    for (i = 0; (i < packageNameSections.length)
                            && (i < groupIdSections.length); i++) {
                        if (!packageNameSections[i].equals(groupIdSections[i])) {
                            break;
                        }
                    }
                    groupIdSections = new String[i];
                    System.arraycopy(packageNameSections, 0, groupIdSections,
                            0, i);
                }
            }
    ...
    public String getBundleFileName(Artifact artifact) throws IOException {
        return getBundleSymbolicName(artifact) + "_"
                + getVersion(artifact.getVersion()) + ".jar";
    }
    ...
    public String getVersion(String version) {
        ...
        Pattern DATED_SNAPSHOT = Pattern
        ...
        Pattern DOTS_IN_QUALIFIER = Pattern
        ...
        if (m.matches()) {
            ...
            /*
             * if last portion before dot is only numbers then it's not in the
             * middle of the qualifier
             */
        ...
        /*
         * convert 1.string -> 1.0.0.string 1.2.string -> 1.2.0.string 1 ->
         * 1.0.0 1.1 -> 1.1.0
         */
        // Pattern NEED_TO_FILL_ZEROS = Pattern.compile(
        // "([0-9])(\\.([0-9]))?\\.([0-9A-Za-z_-]+)" );
        Pattern NEED_TO_FILL_ZEROS = Pattern
                .compile("([0-9])(\\.([0-9]))?(\\.([0-9A-Za-z_-]+))?");
